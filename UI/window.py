...
class PPMWindow:
    ...
    controller = None
    screensize = '480x360'
    # lock image (base64-encoded)
    imageData = PPMLogo.imageData
...
class InitialWindow(PPMWindow):
    ...
    def start(self):
        ...
        self.im = PhotoImage(data=super().imageData)
        Label(super().window, image=self.im).pack()
...
class PasswordVaultWindow(PPMWindow):
    ...
    def start(self):
        ...
        self.search_button = Button(
            super().window,
            text="Search",
            command=super().controller.search
        )
        ...
        self.refresh = Button(
            ...
            bg="green",
            fg="white",
            command=super().controller.refreshall
        )
        ...
        if super().controller.entryman.entries is not None:
            for i in range(len(super().controller.entryman.entries)):
                UserEntry(super().controller.entryman.entries, i)
